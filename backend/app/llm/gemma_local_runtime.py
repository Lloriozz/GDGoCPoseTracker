...
class LocalGemmaInferencer(BaseLLMBackend):
    ...
    def _cleanup_response(self, text: str, prompt: dict[str, object]) -> str:
        ...
        # Remove thinking sequences generated by reasoning models
        raw = re.sub(r"(?is)<\|channel\|>thought.*?<\|channel\|>(assistant|model)\s*", "", raw)
        if "<|channel|>thought" in raw:
            raw = re.sub(r"(?is)<\|channel\|>thought.*", "", raw)
