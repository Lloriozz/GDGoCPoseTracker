...
export default function BicepWorkoutScreen() {
  ...
  const {
    ...
    resetWorkout,
    playSound
  } = useWorkoutTimer({ workoutData: BICEP_WORKOUT_DATA });
  ...
  const currentExercise = BICEP_WORKOUT_DATA.exercises[currentExerciseIndex];
  const nextExercise = currentExerciseIndex + 1 < BICEP_WORKOUT_DATA.exercises.length
    ? BICEP_WORKOUT_DATA.exercises[currentExerciseIndex + 1]
    : null;
      ...
      <View style={workoutStyles.header}>
        <View style={{ flex: 1 }}>
          <Text style={[workoutStyles.progressText, phase === 'rest' && { color: 'rgba(255,255,255,0.7)' }]}>
            Exercise {currentExerciseIndex + 1} of {BICEP_WORKOUT_DATA.exercises.length}
          </Text>
        </View>
